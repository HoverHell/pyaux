...
class Url(DotDict):
    ...
    def to_string(self):
        ...
        return urllib.parse.urlunparse(self[key] if key != "query" else query_str for key in self._base_components)
...
def displaydf(df, *ar, **kwa):
    ...
    if cutlinks:
        ...
        def cutlink(link):
            return f'<a href="{link}">{_cut(link, cutlinks)}</a>'
...
def _re_largest_matching_start(regex, value, *, return_regexp=False):
    ...
    all_match_tries = ((subreg, _try_match(subreg, substr)) for subreg in all_regexes for substr in all_substrings)
